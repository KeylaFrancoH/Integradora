...
router.post("/", async (req, res) => {
  ...
  if (
    ...
    return res
      ...
      .json({
        error:
          "Datos incompletos: idConfiguracion, puntos_x, y puntos_y son requeridos",
      });
  }
...
router.get("/configuracion/:idConfiguracion", async (req, res) => {
  try {
    const idConfiguracion = req.params.idConfiguracion;
    const registros = await Seleccion.findAll({
      where: { idConfiguracion },
    });
    res.json(registros);
  } catch (error) {
    console.error("Error al obtener registros por idConfiguracion:", error);
    res
      .status(500)
      .json({ error: "Error al obtener registros por idConfiguracion" });
  }
});
...
router.put("/:id", async (req, res) => {
  ...
  try {
    const [updated] = await Seleccion.update(
      { idParametro, nombre_metodo },
      {
